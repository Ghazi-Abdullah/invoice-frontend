  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 class="text-3xl font-bold text-gray-900">{{ $t('invoices.invoiceDetails') }}</h1>
          <p class="text-gray-600 mt-2">{{ $t('invoices.viewDetails') }}</p>
        </div>
      ...
      <div v-else-if="!invoice" class="text-center py-12">
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-2">{{ $t('invoices.noInvoice') }}</h3>
        <p class="text-gray-500">{{ $t('invoices.notFound') }}</p>
      </div>
      ...
      <div v-else class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div class="px-6 py-4 bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <h2 class="text-2xl font-bold">{{ $t('invoices.invoice') }} #{{ invoice.invoice_number }}</h2>
        ...
        <div class="p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('clients.clientInfo') }}</h3>
              <div class="space-y-2">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="font-medium text-gray-900">{{ invoice.client?.name || $t('clients.notProvided') }}</p>
                    <p class="text-gray-600 text-sm">{{ invoice.client?.email || $t('clients.noEmail') }}</p>
                  </div>
                </div>
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('invoices.dates') }}</h3>
              <div class="space-y-3">
          ...
          <!-- Invoice Items -->
          <div class="mb-8">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('invoices.items') }}</h3>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-4 py-3 text-right text-sm font-medium text-gray-700">{{ $t('invoices.description') }}</th>
                    <th class="px-4 py-3 text-right text-sm font-medium text-gray-700">{{ $t('invoices.quantity') }}</th>
                    <th class="px-4 py-3 text-right text-sm font-medium text-gray-700">{{ $t('invoices.unitPrice') }}</th>
                    <th class="px-4 py-3 text-right text-sm font-medium text-gray-700">{{ $t('invoices.total') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="item in invoice.items" :key="item.id">
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900">{{ item.unit_price }} {{ $t('common.currency') }}</td>
          ...
          <div class="border-t border-gray-200 pt-6">
            <div class="max-w-xs ml-auto space-y-3">
              <div class="flex justify-between">
                ...
                <span class="font-medium">{{ invoice.subtotal }} {{ $t('common.currency') }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">{{ $t('invoices.tax') }} (15%):</span>
                <span class="font-medium">{{ invoice.tax_amount }} {{ $t('common.currency') }}</span>
              </div>
              <div class="flex justify-between border-t border-gray-200 pt-3">
                <span class="text-lg font-semibold text-gray-900">{{ $t('invoices.totalAmount') }}:</span>
                <span class="text-lg font-bold text-blue-600">{{ invoice.total_amount }} {{ $t('common.currency') }}</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="invoice.notes" class="mt-8 p-4 bg-gray-50 rounded-lg">
            <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ $t('invoices.notes') }}</h3>
...
export default {
  ...
  computed: {
    // يمكنك إضافة computed properties إذا لزم الأمر
  },
  methods: {
    ...
    getDaysRemaining(dueDate) {
      ...
      return `${diffDays} ${this.$t('invoices.days')}`
    },
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent('show-toast', {
            detail: {
              type: 'success',
              message: this.$t('invoices.statusUpdated'),
            },
          }),
        )
        ...
        window.dispatchEvent(
          new CustomEvent('show-toast', {
            detail: {
              type: 'error',
              message: this.$t('invoices.statusUpdateFailed'),
            },
          }),
        )
      }
    ...
    downloadPDF() {
      ...
      window.dispatchEvent(
        new CustomEvent('show-toast', {
          detail: {
            type: 'info',
            message: this.$t('invoices.pdfComingSoon'),
          },
        }),
      )
    },
    ...
    async loadInvoice() {
      ...
      try {
        ...
        if (!invoiceId) {
          throw new Error(this.$t('invoices.invalidId'))
        }
        ...
        if (!this.invoice) {
          this.error = this.$t('invoices.notFound')
        }
      } catch (error) {
        console.error('❌ Failed to fetch invoice:', error)
        this.error = error.message || this.$t('invoices.loadFailed')
      } finally {
        this.loading = false
      }
    },
