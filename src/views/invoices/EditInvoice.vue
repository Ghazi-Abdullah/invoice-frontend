        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <form @submit.prevent="submitInvoice" class="p-6 space-y-6">
            ...
            <div>
              ...
              <!-- ✅ الفرع: يسمح بنقل الفاتورة لفرع آخر - القائمة مبنية من
                   الفروع المسموح بها لهذا المستخدم فقط (branch/branches) -->
              <div class="space-y-2 mt-5">
                <label class="block text-sm font-medium text-gray-700">{{
                  $t('branch.title') || 'الفرع'
                }}</label>
                <select
                  v-model="form.branch_id"
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200"
                >
                  <option :value="null">{{ $t('branch.unassigned') || 'بدون فرع' }}</option>
...
export default {
  ...
  mounted() {
    this.loadInvoice()
    // قد تكون هذي الصفحة أول صفحة تُفتح قبل ما BranchSelector (بالنافبار)
    // يجيب الفروع - خصوصاً على الموبايل وين BranchSelector مخفي أصلاً
    ...
    async loadInvoice() {
      ...
      if (this.invoice) {
        this.form = {
          ...
          branch_id: this.invoice.branch_id || null,
