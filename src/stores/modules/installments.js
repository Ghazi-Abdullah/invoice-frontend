...
const actions = {
  async fetchPlan({ commit }, invoiceId) {
    ...
    try {
      const res = await axios.get(`/invoices/${invoiceId}/installment-plan`);
      ...
      commit('SET_ERROR', e.response?.data?.message || 'فشل في جلب الخطة');
      throw e;
    } finally {
  ...
  async createPlan({ commit }, { invoiceId, data }) {
    ...
    try {
      const res = await axios.post(`/invoices/${invoiceId}/installment-plan`, data);
      commit('SET_CURRENT_PLAN', res.data.data);
      return res.data.data;
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في إنشاء الخطة');
      throw e;
    } finally {
  ...
  async payInstallment({ commit }, { installmentId, paymentMethod = 'cash' }) {
    commit('SET_SUBMITTING', true);
    try {
      const res = await axios.put(`/installments/${installmentId}/pay`, { payment_method: paymentMethod });
      commit('UPDATE_INSTALLMENT', res.data.data);
      return res.data.data;
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في سداد القسط');
      throw e;
    } finally {
      commit('SET_SUBMITTING', false);
    }
  ...
  async cancelPlan({ commit }, planId) {
    commit('SET_SUBMITTING', true);
    try {
      const res = await axios.delete(`/installment-plans/${planId}`);
      commit('SET_CURRENT_PLAN', res.data.data);
      return res.data.data;
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في إلغاء الخطة');
      throw e;
    } finally {
      commit('SET_SUBMITTING', false);
    }
  ...
  async suggestRate({ commit }, numberOfInstallments) {
    try {
      const res = await axios.get(`/installment-interest-tiers/suggest/${numberOfInstallments}`);
      return res.data.data.interest_rate;
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في جلب النسبة المقترحة');
      return 0;
    }
  },
  async fetchTiers({ commit }) {
    commit('SET_LOADING', true);
    try {
      const res = await axios.get('/installment-interest-tiers');
      commit('SET_INTEREST_TIERS', res.data.data);
      return res.data.data;
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في جلب النسب');
      throw e;
    } finally {
      commit('SET_LOADING', false);
    }
  ...
  async saveTier({ commit, dispatch }, { numberOfInstallments, interestRate }) {
    commit('SET_SUBMITTING', true);
    try {
      await axios.post('/installment-interest-tiers', {
        number_of_installments: numberOfInstallments,
        interest_rate: interestRate,
      });
      await dispatch('fetchTiers');
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في حفظ النسبة');
      throw e;
    } finally {
      commit('SET_SUBMITTING', false);
    }
  ...
  async deleteTier({ commit, dispatch }, tierId) {
    commit('SET_SUBMITTING', true);
    try {
      await axios.delete(`/installment-interest-tiers/${tierId}`);
      await dispatch('fetchTiers');
    } catch (e) {
      commit('SET_ERROR', e.response?.data?.message || 'فشل في حذف النسبة');
      throw e;
    } finally {
      commit('SET_SUBMITTING', false);
    }
